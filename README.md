...
- **`blinkTheme`** is Pulse / NeverBlink's own design system (Source Sans Pro). Unlike the other two it is not a replica of somebody else's system - it is ours, which is why it is the one theme that covers **all of MUI** rather than one design system's slice of it. Where the design system has no equivalent component, the style is derived from blink's own tokens and from the decisions its existing components already make, so dropping the theme in never leaves a stock-Material control on the page. **Light only so far** - the theme is written in the shape that makes adding a dark scheme a one-line change, but it is not covered yet, so there is deliberately no `blink-dark` project in the harness.
...
| The SHAPE of the pop transition on Popover, Menu and Dialog | The kit fades and scales from 0.95 through a react-transition-group component passed in `slots`. The theme's contract is that it imports only `@mui/material`, `lucide-react` and React, so a component cannot ship in it, and MUI's own Grow stands in. The settled overlay is identical; an app that wants the exact entrance passes the same prop the kit does. Its TIMING is carried, because timing is a prop - Popover and Menu take the kit's `{ enter: 150, exit: 0 }`, so they open over 150ms and close on the spot |
