...
export const kumoTheme = createTheme({
  ...
  components: {
    ...
    // The caption bar over a tile. Kumo has no image list and no scrim of any kind, so the fill is
    // its `contrast` token at 60% - the token it reserves for content that has to read against an
    // arbitrary background - rather than MUI's own black gradient.
    MuiImageListItemBar: {
      styleOverrides: {
        root: ({ theme }) => ({
          backgroundColor: `color-mix(in oklab, ${theme.vars.palette.kumo.contrast} 60%, transparent)`,
          color: theme.vars.palette.kumo.textInverse,
        }),
        title: { ...TEXT_SM, fontWeight: 500, letterSpacing: "normal" }, // kumo: text-sm font-medium
        subtitle: { ...TEXT_XS, letterSpacing: "normal" }, // kumo: text-xs
      },
    },
    // The trailing slot on a list row. Kumo's equivalent is Sidebar.MenuBadge, which is simply
    // pushed to the end of the row (`inline-flex shrink-0 items-center`) - so this only has to stop
    // MUI absolutely positioning it, which would take it out of the row's flex flow.
    MuiListItemSecondaryAction: {
      styleOverrides: {
        root: {
          position: "static" as const,
          transform: "none",
          display: "inline-flex", // kumo: inline-flex items-center
          alignItems: "center",
          flexShrink: 0, // kumo: shrink-0
          marginLeft: "auto",
        },
    ...
    // The small buttons a SpeedDial fans out. Composed from the pieces Kumo does have: the Button's
    // circle shape at the small size row (h-6.5 => 26px) over the secondary variant's surface.
    MuiSpeedDialAction: {
      styleOverrides: {
        fab: ({ theme }) => ({
          width: "26px", // kumo: the small size row's h-6.5
          height: "26px",
          minHeight: 0,
          borderRadius: "9999px", // kumo: the circle shape's rounded-full
          backgroundColor: theme.vars.palette.kumo.base, // kumo: secondary's bg-kumo-base
          color: theme.vars.palette.kumo.textDefault, // kumo: !text-kumo-default
          boxShadow: ringWith(theme.vars.palette.kumo.line), // kumo: ring ring-kumo-line + shadow-xs
          "&:hover": {
            backgroundColor: theme.vars.palette.kumo.tint, // kumo: not-disabled:hover:bg-kumo-tint
            boxShadow: ringWith(theme.vars.palette.kumo.line),
          },
        }),
        staticTooltipLabel: ({ theme }) => ({
          ...TEXT_SM, // kumo: text-sm
          letterSpacing: "normal",
          backgroundColor: theme.vars.palette.kumo.control,
          color: theme.vars.palette.kumo.textDefault,
          borderRadius: "6px", // kumo: rounded-md
          boxShadow: ringWith(theme.vars.palette.kumo.line),
        }),
