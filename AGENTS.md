...
- While iterating on ONE component, filter to it for a ~5s loop instead of the full suite: `PARITY_PAIR=slider pnpm exec playwright test e2e/parity.spec.ts --project=shadcn-light` (comma-separated id prefixes; add `--project=shadcn-dark` or drop it for both). Run the full `pnpm verify:parity` once at the end to confirm no regressions.
